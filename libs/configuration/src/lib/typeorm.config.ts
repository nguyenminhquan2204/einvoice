...
export class TypeOrmConfiguration {
  ...
  constructor(data?: Partial<TypeOrmConfiguration>) {
    ...
    this.DATABASE = data?.DATABASE || process.env['TYPEORM_DATABASE'] || 'einvoice_local';
...
export const TypeOrmProvider = TypeOrmModule.forRootAsync({
  ...
  useFactory: async (configSerive: ConfigService) => {
    return {
      type: configSerive.get<string>('TYPEORM_CONFIG.TYPE') as DatabaseType,
      host: configSerive.get<string>('TYPEORM_CONFIG.HOST'),
      port: configSerive.get<number>('TYPEORM_CONFIG.PORT'),
      username: configSerive.get<string>('TYPEORM_CONFIG.USERNAME'),
      password: configSerive.get<string>('TYPEORM_CONFIG.PASSWORD'),
      database: configSerive.get<string>('TYPEORM_CONFIG.DATABASE'),
